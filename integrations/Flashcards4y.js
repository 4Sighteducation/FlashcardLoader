         ...
         try {
             ...
             switch (type) {
                 ...
                 case 'full': // This is the primary case used now
                     ...
                     // --- ADD HANDLING FOR TOPIC LISTS --- 
                     if (data.topicLists !== undefined) {
                        updateData[FIELD_MAPPING.topicLists] = JSON.stringify(this.ensureSerializable(data.topicLists || []));
                        // Encode after stringifying
                        updateData[FIELD_MAPPING.topicLists] = safeEncodeURIComponent(updateData[FIELD_MAPPING.topicLists]);
                        dlog("[SaveQueue] Stringified topicLists for 'full' save.");
                     }
                     // --- END ADD HANDLING FOR TOPIC LISTS --- 
                     break;
                 default:
                     ...
                     if (iframe && iframe.contentWindow) { 
                         const initialData = {
                             ...
                             data: {
                                 ...
                                 // Corrected: Use the lastSaved timestamp from the flashcard userData, not the main user record
                                 knackRecordLastSaved: userData?.lastSaved || null 
                             }
                         };
                         debugLog("--> Sending KNACK_USER_INFO to React App (with corrected knackRecordLastSaved from flashcard data)", initialData.data);
    ...
    dlog("[Knack Script] Handling ADD_TO_BANK request");
     ...
     try {
         ...
         // --- Prepare Box 1 Update ---
          ...
          // Calculate the correct review date (1ms before start of current UTC day)
          const reviewDateForNewCard = new Date(new Date().setUTCHours(0,0,0,0) - 1).toISOString();
          ...
          const newBox1Entries = cardsToAdd
            ...
            .map(card => ({ 
              cardId: card.id, 
              lastReviewed: nowISO, 
              nextReviewDate: reviewDateForNewCard // <-- Corrected line
            }));
