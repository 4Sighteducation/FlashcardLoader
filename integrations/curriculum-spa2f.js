...
class CurriculumAPI {
    ...
    buildSupabaseActivity(record) {
        // Prefer normalized table columns first to avoid shadow/legacy JSON leaking into UI filters.
        const book = record.book || record.content?.book || '';
        const month = record.month || record.content?.month || '';
