    ...
    window.StudentEmulator = {
      ...
      createEmulationModal: function(student) {
            ...
            <div class="se-modal-content se-emulation">
              ...
              <div class="se-iframe-container">
                <div class="se-loading">
                  ...
                  <div class="se-manual-login" style="display: none; margin-top: 20px;">
                    <p style="color: #666;">Having trouble? Use these credentials to log in manually:</p>
                    ...
                    <button onclick="StudentEmulator.showIframeManually()" style="background: #007bff; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer;">Show Portal</button>
                  </div>
                </div>
                <iframe 
        ...
        const checkLoginStatus = () => {
          try {
            ...
            // Check for login form more aggressively
            const hasLoginElements = iframeDoc.querySelector('input[type="password"], input[type="email"], .kn-login, form');
            const currentUrl = iframe.contentWindow.location.href;
            const isLoginUrl = !currentUrl.includes('#') || currentUrl.includes('login') || currentUrl === 'https://vespaacademy.knack.com/vespa-academy' || currentUrl === 'https://vespaacademy.knack.com/vespa-academy/';
            
            if (hasLoginElements && isLoginUrl && !isLoggedIn && loginAttempts < 3) {
              loginAttempts++;
        ...
        // Show manual login option after 10 seconds if still loading
        setTimeout(() => {
          const loadingDiv = document.querySelector('.se-loading');
          const manualLogin = document.querySelector('.se-manual-login');
          if (loadingDiv && loadingDiv.style.display !== 'none' && manualLogin) {
            console.log('[Student Emulator] Auto-login timeout, showing manual option');
      ...
      performLogin: function(student) {
        try {
          ...
          setTimeout(() => {
            // Try multiple selector strategies for Knack's login form
            let emailInput = null;
            let passwordInput = null;
            let submitButton = null;
            
            // Strategy 1: Look for inputs by type within any form
            const allForms = iframeDoc.querySelectorAll('form');
            console.log('[Student Emulator] Found', allForms.length, 'forms');
            
            for (const form of allForms) {
              if (!emailInput) {
                emailInput = form.querySelector('input[type="email"], input[type="text"][name*="email"], input[type="text"][placeholder*="email" i]');
              }
              if (!passwordInput) {
                passwordInput = form.querySelector('input[type="password"]');
              }
              if (!submitButton) {
                submitButton = form.querySelector('button[type="submit"], input[type="submit"], button.kn-button');
              }
            }
            
            // Strategy 2: Look globally if not found in forms
            if (!emailInput) {
              emailInput = iframeDoc.querySelector('input[type="email"], input[name="email"], input#email, input[placeholder*="email" i], .kn-input input[type="text"]');
            }
            if (!passwordInput) {
              passwordInput = iframeDoc.querySelector('input[type="password"], input[name="password"], input#password');
            }
            if (!submitButton) {
              submitButton = iframeDoc.querySelector('button.kn-button, button[type="submit"], input[type="submit"], .kn-submit button');
            }
            ...
            if (emailInput && passwordInput) {
              ...
              // Use a more robust way to set values
              ...
              setTimeout(() => {
                setNativeValue(emailInput, student.email);
                emailInput.dispatchEvent(new Event('change', { bubbles: true }));
                emailInput.blur();
                
                // Set password after email
                setTimeout(() => {
                  passwordInput.focus();
                  setNativeValue(passwordInput, '');
                  setTimeout(() => {
                    setNativeValue(passwordInput, student.password);
                    ...
                    setTimeout(() => {
                      if (submitButton && !submitButton.disabled) {
                        ...
                        // Alternative: try to submit the form
              ...
              console.log('[Student Emulator] Login form not found yet');
              ...
              if (allInputs.length > 0 && allInputs.length < 10) {
                // Only log details if reasonable number of inputs
                allInputs.forEach((input, index) => {
                  console.log(`[Student Emulator] Input ${index}:`, {
                    ...
                    className: input.className.substring(0, 50) // Limit class name length
                  });
                });
              }
              
              // Check if we might be on a different page
              const pageTitle = iframeDoc.title;
              const bodyText = iframeDoc.body ? iframeDoc.body.innerText.substring(0, 200) : '';
              console.log('[Student Emulator] Page title:', pageTitle);
              console.log('[Student Emulator] Body preview:', bodyText);
            }
      ...
      // Show iframe manually (fallback option)
      showIframeManually: function() {
        const loadingDiv = document.querySelector('.se-loading');
        const iframe = document.getElementById('student-portal-iframe');
